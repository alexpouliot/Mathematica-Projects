...
RbLine[A_,G_,H_,P_,T_,Br_,Sh_,\[Nu]p_]:=levelLookup[A,G,H]Voigt[(Subscript[\[CapitalGamma]N, A]+Br P)/2,Sqrt[(kB T)/Subscript[mRb, A]] Subscript[\[Nu], A,G,H]/c,Subscript[\[Nu], 85,3,4]+\[Nu]p-Subscript[\[Nu], A,G,H]-Sh P];
RbLineFree[A_,G_,H_,T_,\[CapitalGamma]P_,\[Delta]P_,\[Nu]p_]:=levelLookup[A,G,H]Voigt[(Subscript[\[CapitalGamma]N, A]+\[CapitalGamma]P)/2,Sqrt[(kB T)/Subscript[mRb, A]] Subscript[\[Nu], A,G,H]/c,Subscript[\[Nu], 85,3,4]+\[Nu]p-Subscript[\[Nu], A,G,H]-\[Delta]P];
NatRbLines[nTot_,len_,P_,T_,Br_,Sh_,\[Nu]p_]:=nTot len Sum[RbLine[A,G,H,P,T,Br,Sh,\[Nu]p],{A,{85,87}},{G,Subscript[Gs, A]},{H,Subscript[Hs, G]}];
NatRbLinesFree[nTot_,len_,T_,\[CapitalGamma]P_,\[Delta]P_,\[Nu]p_]:=nTot len Sum[RbLineFree[A,G,H,T,\[CapitalGamma]P,\[Delta]P,\[Nu]p],{A,{85,87}},{G,Subscript[Gs, A]},{H,Subscript[Hs, G]}];
...
Subscript[Hs, G_]=Range[G-1,G+1]; (*Excited states accesible by electric dipole transistions from ground state G*)
Subscript[g, j_]:=2j +1; (*no. of substates in state with max ang. momentum j*)


(*Total oscillator strength for F=G->F'=H D2 (L=0 to L=1/2) transition, with nuclear ang. mom. Subscript[II, A] in units of |\[LeftAngleBracket]J=1/2||e r||J'=3/2\[RightAngleBracket]|^2*)
Off[ClebschGordan::phy];
Subscript[S, A_,G_,H_]:=Sum[((-1)^(H+1-mG) Sqrt[(2G +1)(2H+1)]ThreeJSymbol[{G,-mG},{1,q},{H,mH}]SixJSymbol[{G,1,H},{3/2,Subscript[II, A],1/2}])^2,{q,-1,1,2},{mG,-G,G},{mH,-H,H}]; 
...
Subscript[\[Nu], A_,G_,H_] :=Which[A==85,
		Which[G==2,
...
Subscript[II, A_]:=Which[A==85,5/2,A==87,3/2]; (*Nuclear spin of each nat. isotope*)
(*Steck, natural linewidth in Hz for Rb (multiply by 2\[Pi] for decay rate)*)
Subscript[\[CapitalGamma]N, A_]:=Which[A==85,6.0666 10^6,
A==87,6.065 10^6]; 
Subscript[mRb, A_]:= m0 A; (*mass of isotope A in kg*)
Subscript[Gs, A_]:=Which[A==85,{2,3},A==87,{1,2}]; (*ground states for nat. isotopes of Rb*)
Subscript[Ab, A_,G_]=Which[A==85,Subscript[g, G]/Sum[Subscript[g, F],{F,Subscript[Gs, 85]}] 0.722,A==87,Subscript[g, G]/Sum[Subscript[g, F],{F,Subscript[Gs, 87]}] 0.278];(*Natural abundance with each ground state sublevel equally populated*)
Subscript[inPJ, A_]:=Which[A==85,4.2275362 a0 e,A==87,4.2275262 a0 e]; (*inner product \[LeftAngleBracket]J=1/2||e r||J'=3/2\[RightAngleBracket] in all directions*)
...
Subscript[f, A_,G_,H_]:=(2 Subscript[mRb, A] Subscript[\[Nu], A,G,H])/(3 \[HBar] Subscript[g, G]) Subscript[S, A,G,H] Subscript[inPJ, A]^2;(*oscillator strength*)
Subscript[exConst, A_]:= \[Pi]/(2 \[Epsilon]0 Subscript[mRb, A] c);(*coefficient in absorbtion exponential *)
(*Make lookup table for product of entire exponential coefficient to make functions more efficient*)
coTab=Table[Subscript[exConst, A] Subscript[Ab, A,G] Subscript[f, A,G,H],{A,{85,87}},{G,Subscript[Gs, A]},{H,Subscript[Hs, G]}];
